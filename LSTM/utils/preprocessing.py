...
def dataset_vectorizing(data_set_X, data_set_y, LSTM_window_left, LSTM_window_right, higher_bound):
    ...
    for i in range(LSTM_window_left, higher_bound):
        ...
        X_data3.append(data_set_X[i-LSTM_window_left:i+LSTM_window_right+1, [3]].reshape(-1, 1))
        X_data4.append(data_set_X[i-LSTM_window_left:i+LSTM_window_right+1, [4]].reshape(-1, 1))
    ...
    X_data3 = np.reshape(X_data3, (X_data3.shape[0], X_data3.shape[1], 1))
    X_data4 = np.array(X_data4)
    X_data4 = np.reshape(X_data4, (X_data4.shape[0], X_data4.shape[1], 1))
    X_data5 = np.array(X_data5)
    X_data5 = np.reshape(X_data5, (X_data5.shape[0], X_data5.shape[1], 1))
    y_data = np.array(y_data)

    # X_data_total.shape = (higher_bound - LSTM_window_left) x (LSTM_window_left + LSTM_window_right + 1) x 3
    X_data_total = np.concatenate((X_data0, X_data1, X_data2, X_data3, X_data4, X_data5), axis=2).astype('float16')
